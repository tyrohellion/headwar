<script>
	...
	const isOps = $derived(abbr.toUpperCase() === 'OPS' || abbr.toUpperCase() === 'OOPS');
	const isEra = $derived(abbr.toUpperCase() === 'ERA');
	const isWHIP = $derived(abbr.toUpperCase() === 'WHIP');
	const isKPER9 = $derived(abbr.toUpperCase() === 'K/9');
	const isWalks = $derived(abbr.toUpperCase() === 'BB/9');
	const isOBP = $derived(abbr.toUpperCase() === 'OOBP' || abbr.toUpperCase() === 'OBP');
	...
	const activeColor = $derived.by(() => {
		...
		if (isOBP) {
			if (numericValue <= 0.29) return 'var(--wa-color-success-60)';
			if (numericValue <= 0.32) return 'var(--wa-color-success-80)';
			if (numericValue <= 0.345) return 'var(--wa-color-neutral-50)';
			return 'var(--wa-color-danger-70)';
		}
