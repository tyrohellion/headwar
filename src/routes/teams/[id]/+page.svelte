...
<svelte:head>
  <title>{teamProfile?.name ?? "headwar"}</title>
</svelte:head>
  ...
  <div class="player-info-box">
    ...
    <div class="player-text-box">
      ...
      <div class="small-details-wrapper">
        <wa-badge appearance="filled" variant="brand"
          >{teamProfile.league?.name}</wa-badge
        >
      </div>
    </div>
  </div>
  ...
  {#key isDesktop}
    <wa-tab-group placement={isDesktop ? "start" : "top"}>
      ...
      <wa-tab-panel name="pitching"
        >This is the pitching tab panel.</wa-tab-panel
      >
      <wa-tab-panel name="prospects"
        >This is the prospects tab panel.</wa-tab-panel
      >
    </wa-tab-group>
  {/key}
{/if}
